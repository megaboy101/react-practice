...
// import reduxImmutableStateInvariant from 'redux-immutable-state-invariant'; <-- Taken out for production build, just used for dev warnings
...
export default function configureStore(initialState) { // Initialize store with initial states
	return createStore(
		rootReducer,
		initialState,
		applyMiddleware(thunk) // Infinite parameters for every middleware to add
	);
}
